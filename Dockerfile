...
RUN apt-get update && apt-get install --no-install-recommends -y pkg-config libglib2.0-dev clang-tools-9 vim libc6-dev make dpkg-dev python3.8 python2.7 ninja-build git curl software-properties-common && \
...
cd / && git clone https://gn.googlesource.com/gn && cd gn && python build/gen.py && ninja -C out && cp -f out/gn $GN_BIN_PATH && \
cd /tmp && fetch v8 && \
